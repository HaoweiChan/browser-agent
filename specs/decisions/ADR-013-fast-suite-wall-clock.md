...
**Ruling**: [historical] ADR-002 Decision 4's `fast` wall-clock ceiling is per-environment and each number is measured as the slowest observed run +15% (CI) or as the slowest *reproducible* run (local, since round 5 — see Decision 4) — 60s locally and 80s on CI at the time of writing; both have moved since, and the numbers in force are ADR-019 §2/§3/§5 as amended by ADR-021, read through `EVAL_WALL_BUDGET_S_FAST` and committed in `evals/run.py` rather than restated here — applied by `evals/run.py` to the run it just measured, exiting non-zero; the suite gets one shared Chromium, re-launched if it dies, with each run in its own BrowserContext.
...
**Amended by**: [historical] ADR-019 (Decision 4's local number re-derived 60 -> 80 after M31 grew the suite; `invariant` given its own 20s ceiling; CI's `fast` number re-measured on CI to 90s; `EVAL_WALL_BUDGET_S` split into one variable per suite; every band computed from the committed ledger and graded against it); **ADR-021** (ADR-019's local `fast` number re-derived 80 -> 90 at 146 cases, reaching this ADR's Decision 4 through ADR-019 Decision 2)
