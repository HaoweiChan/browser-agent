...
```bash
python3 -m evals.run --suite fast        # offline gate: 160 cases, zero paid calls
...
Latest offline baseline — `evals/report/20260824-014443-fast.json`, with
`evals/report/20260824-014332-invariant.json` and
`evals/report/20260823-164737-live.json`:

```
fast  160/160    invariant  63/63    live  9/9    $0.0000    70.6s
...
|---|---|---|---|---|
| `fast` | 160 | 71.1s | 81.76 | **90s** |
| `invariant` | 63 | 13.76s | 15.82 | **20s** |
...
The eval set is not weak; it is 171 cases (160 of them in the offline gate), it
