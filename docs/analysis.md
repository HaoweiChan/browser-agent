...
277 distinct cases (28 golden + 249 adversarial).
459 browser actions in a `fast` run; **162 of the 248** `fast` cases drive a real Chromium end to end — counted here as
cases that actually recorded browser actions, read out of the committed report
`evals/report/20260828-093157-fast.json` rather than tallied by hand (the
...
remaining 86 are those refusals plus pure-code probes of a single
