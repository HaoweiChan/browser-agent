...
```bash
python3 -m evals.run --suite fast        # offline gate: 175 cases, zero paid calls
...
Latest offline baseline — `evals/report/20260824-013053-fast.json`, with
`evals/report/20260824-013106-invariant.json` and
`evals/report/20260823-164737-live.json`:

```
fast  175/175    invariant  61/61    live  9/9    $0.0000    73.1s
...
|---|---|---|---|---|
| `fast` | 175 | 73.04s | 84.0 | **90s** |
| `invariant` | 61 | 14.08s | 16.19 | **20s** |
...
The eval set is not weak; it is 186 cases (175 of them in the offline gate), it
