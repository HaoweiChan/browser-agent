...
| Replan rate | replans per task | trace, plus `budgets_spent.replans` (the plan lint and the `act` ladder share the counter — M31, ADR-018) | — | architecture-honesty alarm: if most tasks replan constantly, the "evolving prefix" claim is re-examined in the ADR. The lint's replan is charged here but is NOT labelled `recovery`: nothing failed and no ladder ran, so it stays out of the recovery metric by construction |
