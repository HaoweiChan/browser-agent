...
Date: 2026-08-16
Status: accepted; all six decisions stand as written. **Decision 4 (fast wall clock ≤ 60s [historical]) was knowingly breached from M8 to M12** — the gate measured 66.6-68.3s — and the breach is **closed at M12 without moving the ceiling**: `specs/decisions/ADR-013-fast-suite-wall-clock.md` measured 11.3s of the 67.0s as per-case browser process lifecycle, removed it, and left the deliberate 42.2s of timeouts untouched. **Decision 4 is also amended in scope**: the ceiling is per-environment since ADR-013 Decision 3, and per-suite as well as per-environment since ADR-019 §4 (`EVAL_WALL_BUDGET_S_FAST`, `EVAL_WALL_BUDGET_S_INVARIANT`) — all four numbers measured, all four enforced, and none of them stated here: they are ADR-019 §2/§3/§5 as amended by ADR-021, and `evals/run.py` commits the local pair. The scope change was forced because CI had been running `fast` in 89.62s against this ADR's original 60s for its entire existence with nothing checking. ADR-013 Decision 4 tried re-measuring the local number to 70s on the M9-stage-2 merge, then withdrew that the same day on round-5 review when the band behind it did not reproduce; the local number ended M12 unchanged at 60s and has been re-derived twice since (ADR-019, ADR-021) from the committed ledger's rows tagged `env: local` — the qualifier matters, because since T-R44 a row carries an `env` tag (`local`, `ci`, or whatever `EVAL_ENV` names) recording which machine measured it, and a local ceiling derived from `ci` rows, or the reverse, is the exact mistake ADR-019 §3 was written to stop. Rows written before T-R44 carry no tag; ADR-012 has the schema and ADR-019 §7 the ordering rule that goes with it. Enforced by `evals/run.py` and graded by `fast-wall-clock-budget` rather than asserted in this file.

**Amended by**: [historical] ADR-009 (Decision 4's 60s wall-clock ceiling, declared breached rather than reset); ADR-013 (that breach closed, enforcement added, the ceiling made per-environment in its Decision 3; its Decision 4 tried and withdrew a 60s -> 70s local re-measurement the same day); **ADR-019** (the local number re-derived 60 -> 80 after M31 grew the suite, a second suite — `invariant` — given a ceiling of its own at 20s, CI's `fast` number re-measured on CI to 90s, and every band computed from the committed ledger rather than by hand); **ADR-021** (per-case cost growth is answered by removing waste, case-COUNT growth is what licenses a raise); ADR-029, ADR-035, ADR-037 and ADR-039 (four further re-derivations of the same local pair by the same rule). **This list is kept current and deliberately carries no numbers**: it names which decisions moved the ceilings, and ADR-019 §2/§3/§5 is where the values live — this ADR publishing its own copy is what T-M32-8 was filed for.

**Ruling**: Sets the pre-commit gate: `fast` suite ≥ the committed baseline (1.000), `invariant` suite = 100% unconditional, trap-catch ≥ 90% as a floor never a verifier-accuracy claim, `fast` suite cost = $0.00 exactly, and each suite's wall clock ≤ its own environment's measured ceiling — **the four numbers are ADR-019 §2/§3/§5, not this ADR's, and `evals/run.py` commits the local pair** (breached M8-M12, restored, made per-environment, a local re-measurement to 70s tried and withdrawn at M12, then re-derived from the committed ledger each time the suite grew — see Amended by; `invariant` has had a ceiling of its own since ADR-019). Decision 4 below keeps its original 60s as the record of what M2 set; it is not the ceiling in force.
