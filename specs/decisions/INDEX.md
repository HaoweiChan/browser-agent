...
- ADR-013 — the reviewer UI uses TinBoker terminal styling without changing its trace-first architecture, stable DOM hooks, single-page delivery, or external-dependency footprint — enforced by `ui-tinboker-style`, `stream-shows-every-step`, `gateway-error-contract-shape`
