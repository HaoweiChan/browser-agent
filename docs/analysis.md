...
176 distinct cases (20 golden + 156 adversarial).
271 browser actions in a `fast` run; **101 of the 165** `fast` cases drive a real Chromium end to end — counted here as
cases that actually recorded browser actions, read out of the committed report
`evals/report/20260823-232554-fast.json` rather than tallied by hand (the
...
176 distinct cases (M32/M38, refreshed from the case files' own `tc`/`level`/`domain`
...
|---|---|---|---|---|
| TC1 extract-on-page | 49 | | L1 | 53 |
| TC2 search-then-extract | 8 | | L2 | 44 |
...
| TC4 interact-then-extract | 35 | | L4 (mutation/recovery) | 16 |
