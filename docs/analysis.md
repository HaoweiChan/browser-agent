...
193 browser actions in a `fast` run; **65 of the
119** `fast` cases drive a real Chromium end to end — counted here as
cases that actually recorded browser actions, read out of the committed report
`evals/report/20260822-170551-fast.json` rather than tallied by hand (the
...
130 distinct cases (M31, refreshed from the case files' own `tc`/`level`/`domain`
...
|---|---|---|---|---|
| TC1 extract-on-page | 30 | | L1 | 35 |
| TC2 search-then-extract | 8 | | L2 | 24 |
| TC3 navigate-then-extract | 11 | | **L3** | **12 — 4 live (one of them unrun) + 8 fixture: the M10 aggregate-superlative twin now caught by M31's plan lint, its green twin `probe3-quotes-most-quoted-author`, `extract-all-refuses-a-selector`, `plan-lint-holds-across-a-midrun-replan`, `extract-all-cheapest-wording-still-reduces`, the PR #29 R16 pair `extract-all-declared-intent-beats-wording` / `extract-all-undeclared-intent-fails-loud`, and R20's `plan-lint-refuses-a-declared-non-comparison`** |
