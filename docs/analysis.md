...
172 distinct cases (20 golden + 152 adversarial).
248 browser actions in a `fast` run; **91 of the 161** `fast` cases drive a real Chromium end to end — counted here as
cases that actually recorded browser actions, read out of the committed report
`evals/report/20260824-013634-fast.json` rather than tallied by hand (the
...
remaining 70 are those refusals plus pure-code probes of a single
...
169 distinct cases (M32, refreshed from the case files' own `tc`/`level`/`domain`
