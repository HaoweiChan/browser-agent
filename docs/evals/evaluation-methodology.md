...
| Replan rate | replans per task | trace, plus `budgets_spent.replans` (the plan lint and the `act` ladder share the counter — M31, ADR-016) | — | architecture-honesty alarm: if most tasks replan constantly, the "evolving prefix" claim is re-examined in the ADR. The lint's replan is charged here but is NOT labelled `recovery`: nothing failed and no ladder ran, so it stays out of the recovery metric by construction |
...
| **Cost/model tradeoff (M9)** | per model over one fixed task set: correct/5, LLM $, $/run, tokens, latency p50/p95 | `evals/ablation.py` drives a **deployed** instance once per model and writes `evals/report/<stamp>-ablation.json`; the table in `docs/analysis.md` §9 is re-derived from that report by `analysis-ablation-table-not-estimated` | hand labels read from the five golden cases the task set names, compared with the production `verifier.answers_match` | one run per (model, task) cell — no variance; five tasks on one prompt, so it measures *this system with that model*, not the model; p95 is the slowest of five; latency is each run's own `budgets_spent.ms`, never the driver's wall clock (queue + poll); `answers_match` is exact outside numbers, so a correct-but-wider extraction scores incorrect. Decision rule fixed in advance: `specs/decisions/ADR-010-m9-model-ablation.md` Decision 5 |
