...
- ADR-010 — ADR-002's 60s `fast` ceiling stands and is now enforced by an eval case; the suite shares one Chromium (own BrowserContext per run) and measures 54.1-55.9s — enforced by `fast-wall-clock-budget`, `agent-launches-its-own-browser` · amends ADR-002, ADR-009
