...
**Amended by**: [historical] ADR-009 (Decision 4's 60s wall-clock ceiling, declared breached rather than reset); ADR-013 (that breach closed, enforcement added, the ceiling made per-environment in its Decision 3; its Decision 4 tried and withdrew a 60s -> 70s local re-measurement the same day); **ADR-019** (the local number re-derived 60 -> 80 after M31 grew the suite, a second suite — `invariant` — given a ceiling of its own at 20s, CI's `fast` number re-measured on CI to 90s, and every band computed from the committed ledger rather than by hand)
