...
list is what it requires, and this file states those requirements nowhere else.
It has to, because three bands in PR #29 did not
...
it (T-R55). It is stated because a band source is taken as it is found — item 2
...
ledger timestamp. §6 items 2-3 are what the check requires of that run; item 4
is about the committed ceiling and is graded against the ledger's maximum, not
against the cited run at all (T-R49). The
...
the check as it now stands — §6 item 5 does not require the rule's value to
...
`[]` (T-R48). What took the state out of reach was item 3, when a 13.32s run
...
**What the check enforces.** One list, in one place: this is the only statement
of what the check requires, and every other sentence in this file and in README
names its items by number rather than repeating what they say. Four rounds of
PR #35 ended the same way — the repair correct, its own description left behind
(R15, R16, R19, R20) — and a second copy of a rule is the thing that goes stale.
Item 8 is how a broken deferral stops being a matter of care: a reference to an
item that is not there is red. What it does NOT catch is a reference that still
resolves and now points at a different rule, which is what renumbering this list
would do to all of them (cold review of T-R56, F2 — logged as T-R59), so
renumber it by appending, never by inserting.
`published-band-matches-the-ledger` requires, per suite for items 1-7 and of
this section itself for item 8:
...
8. this list is numbered 1..N with no gaps, and every `item N` reference in this
   file, in README and in the check's own source names a number the list has —
   under that name, not the `property N` numbering PR #35 round 4 retired. A
   range reference is checked at every number it spans, not only its endpoints.

Green is required nowhere in that list and cannot be (T-R53); item 2 requires
the result to be *stated*, not to be a pass. Item 5 states the rule's value and
...
suite, so items 1-8 have nothing to read; and `no_recorded_run_at` — the ledger
holds no row at the current case count, so item 2 has no candidate.
...
**What a reader should conclude.** Item 2 is why the number beside each band is
...
anything going red. The ceiling beside it is correct either way: item 4 grades
it against that maximum and never against the published number.

Item 2's as-of-the-cited-run reading of cleanliness is deliberate in both
...
which is why item 2 requires the result to be disclosed instead.
