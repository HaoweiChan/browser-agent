...
169 distinct cases (20 golden + 149 adversarial).
254 browser actions in a `fast` run; **94 of the 158** `fast` cases drive a real Chromium end to end — counted here as
cases that actually recorded browser actions, read out of the committed report
`evals/report/20260823-222543-fast.json` rather than tallied by hand (the
...
169 distinct cases (M32/M38, refreshed from the case files' own `tc`/`level`/`domain`
...
|---|---|---|---|---|
| TC1 extract-on-page | 43 | | L1 | 53 |
| TC2 search-then-extract | 8 | | L2 | 37 |
