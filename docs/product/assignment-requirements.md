...
| R4 | Root `prompts/` folder with key prompts — reviewers will read them | Common req. 4 | Curated `prompts/00N-*.md`; convention in `prompts/README.md` | **evidenced** — `prompts/` with 8 curated records |
...
| T3 | Self-correction: diagnose cause on failure, try different strategies | Task 1 bullet 1 | Deterministic failure classifier; class-conditional recovery ladders; recovery metric counts ONLY classify→strategy-switch→verified-success traces (retries excluded) | **evidenced** — deterministic 7-class classifier (`diagnosis 13/13`); 2 ladders; `recovery 3/3 verified (8 rungs tried)` with retries excluded by construction |
...
| T5 | Self-built eval set covering diverse domains and task types | Task 1 ¶2 | `evals/golden/` + `evals/adversarial/`; coverage cells in `docs/evals/evaluation-methodology.md`; case-count matrix with empty cells visible | **evidenced** — 84 cases (20 golden + 64 adversarial); coverage cells in `docs/analysis.md` §6 with the empty L3 row shown |
