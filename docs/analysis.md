...
222 distinct cases (21 golden + 201 adversarial).
373 browser actions in a `fast` run; **134 of the 211** `fast` cases drive a real Chromium end to end — counted here as
cases that actually recorded browser actions, read out of the committed report
`evals/report/20260825-192110-fast.json` rather than tallied by hand (the
...
remaining 77 are those refusals plus pure-code probes of a single
