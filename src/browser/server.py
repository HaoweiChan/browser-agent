...
const ADRS = [
  ...
  ["037", "the observation waits for what the page is still FETCHING — after `load`, navigate waits (bounded, same budget a postcondition gets) for the fetch/xhr requests that were in flight, so a <select> painted from an endpoint is visible to the planner; and a refused identity anchor now replans instead of ending the run"],
