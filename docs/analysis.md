...
294 distinct cases (28 golden + 266 adversarial).
482 browser actions in a `fast` run; **173 of the 262** `fast` cases drive a real Chromium end to end — counted here as
cases that actually recorded browser actions, read out of the committed report
`evals/report/20260828-155048-fast.json` rather than tallied by hand (the
...
remaining 89 are those refusals plus pure-code probes of a single
